...
export default function ExamPageClient() {
  ...
  if (error) return <main className="mx-auto max-w-3xl px-5 pt-16"><p role="alert" className="text-sm text-warn-ink">{error}</p><Link href="/" className="mt-4 inline-block text-sm text-primary-ink">← 서베이 설정</Link><Link href="/topics" className="ml-6 text-sm text-primary-ink">주제별 연습 →</Link></main>;
  if (!exam) return <main className="mx-auto max-w-3xl px-5 pt-16 text-sm text-fg-muted">서베이 문제를 구성하는 중…</main>;
  ...
  if (!started) return <main className="mx-auto w-full max-w-2xl px-5 pb-24 pt-12 sm:px-8">
    ...
    <Card className="mt-5 p-6 sm:p-8">
      ...
      <p className="mt-3 text-sm leading-relaxed text-fg-muted">선택한 서베이 중 3개를 뽑아 Q2~10형을 구성하고, 선택 주제에서 Q14 비교·변화와 Q15 이슈를 추가했습니다.</p>
      <p className="mt-3 text-xs leading-relaxed text-fg-muted">지금은 서베이 연습에만 집중하는 리셋 버전이라 11~13번 롤플레이는 포함하지 않습니다. 번호 분류는 학습 편의를 위한 것이며 실제 시험에서 유형이 항상 같은 번호에 고정되는 것은 아닙니다.</p>
      {exam.notices?.map((notice) => <p key={notice} className="mt-3 text-xs leading-relaxed text-warn-ink">{notice}</p>)}
      <label className="mt-6 flex cursor-pointer items-center gap-3 text-sm text-fg-muted"><input type="checkbox" checked={includeIntro} onChange={(e) => { setIncludeIntro(e.target.checked); build(e.target.checked); }} />1번 자기소개 포함하기</label>
      <div className="mt-7 flex flex-wrap gap-3"><button type="button" onClick={() => setStarted(true)} className="rounded-xl bg-primary px-5 py-3 text-sm font-medium text-primary-fg transition-colors hover:bg-primary-hover">연습 시작</button><button type="button" onClick={() => build(includeIntro)} className="rounded-xl border border-line px-4 py-3 text-sm text-fg-muted">문제 다시 뽑기</button></div>
    </Card>
  </main>;
