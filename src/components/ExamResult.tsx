...
export default function ExamResult({ exam, title, answers, times, onRetry, onRegenerate }: {
  ...
  return <main className="mx-auto w-full max-w-3xl px-5 pb-24 pt-8 sm:px-8">
    ...
    <Card className="animate-fade-up mt-5 p-6 sm:p-8">
      ...
      <p className="mt-2 text-sm leading-relaxed text-fg-muted">문항별 질문과 작성한 답변을 확인하고 다시 말해 보세요.</p>
      <dl className="mt-6 grid grid-cols-3 gap-3 border-t border-line pt-5 text-center">
        ...
        <div><dt className="text-xs text-fg-muted">총 영어 단어 수</dt><dd className="mt-1 text-lg font-medium tabular-nums">{totalWords}</dd></div>
        <div><dt className="text-xs text-fg-muted">총 소요 시간</dt><dd className="mt-1 text-lg font-medium tabular-nums">{formatTime(totalTime)}</dd></div>
      </dl>
      <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={onRetry} className="rounded-xl border border-line px-4 py-2.5 text-sm text-fg-muted">같은 문제 다시 풀기</button>{onRegenerate && <button type="button" onClick={onRegenerate} className="rounded-xl bg-primary px-4 py-2.5 text-sm font-medium text-primary-fg transition-colors hover:bg-primary-hover">새 문제 뽑기</button>}</div>
    </Card>
    <h2 className="mt-10 text-sm font-semibold tracking-widest text-fg-muted">문항별 답변·문제 유형</h2>
...
function ItemResult({ item, answer, elapsed }: { item: ExamItem; answer: string; elapsed: number }) {
  ...
  return <Card className="overflow-hidden">
    <button type="button" aria-expanded={open} onClick={() => setOpen((v) => !v)} className="flex w-full items-center gap-3 px-5 py-4 text-left transition hover:bg-surface-2">
      ...
      <span className="shrink-0 text-xs text-fg-muted">{hasAnswer ? "작성함" : "미작성"}</span>
    ...
    {open && <div className="border-t border-line px-5 py-5">
      ...
      {hasAnswer ? <div className="mt-5 rounded-xl border border-line bg-surface-2 px-4 py-3"><p className="text-[11px] tracking-widest text-fg-subtle">내 답변 · 영어 {countEnglishWords(answer)}단어 · {formatTime(elapsed)}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-fg-muted">{answer}</p></div> : <p className="mt-4 text-sm text-fg-subtle">작성한 답변이 없습니다.</p>}
    </div>}
  </Card>;
}
