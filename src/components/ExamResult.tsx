...
export default function ExamResult({ exam, title, answers, times, hintUse = {}, replays = {}, onRetry, onRegenerate }: {
  ...
  return <main className="mx-auto w-full max-w-3xl px-5 pb-24 pt-8 sm:px-8">
    ...
    <Card className="animate-fade-up mt-5 p-6 sm:p-8">
      ...
      <dl className="mt-6 grid grid-cols-2 gap-3 border-t border-line pt-5 text-center sm:grid-cols-5">
      ...
      <p className="mt-4 text-xs leading-relaxed text-fg-muted">실전에는 지문 보기가 없습니다. 힌트 사용이 0회에 가까워질수록 실제 시험에 가까운 연습입니다.</p>
    ...
    <div className="mt-4 space-y-4">{exam.items.map((item) => <ItemResult key={item.slot} item={item} answer={answers[item.slot] ?? ""} elapsed={times[item.slot] ?? 0} hints={hintUse[item.slot] ?? 0} replays={replays[item.slot] ?? 0} />)}</div>
    <Footer />
  </main>;
}

function ItemResult({ item, answer, elapsed, hints, replays }: { item: ExamItem; answer: string; elapsed: number; hints: number; replays: number }) {
  ...
  return <Card className="overflow-hidden">
    ...
    {open && <div className="border-t border-line px-5 py-5">
      ...
      {hasAnswer ? <div className="mt-5 rounded-xl border border-line bg-surface-2 px-4 py-3"><p className="text-[11px] tracking-widest text-fg-subtle">내 답변 · {countEnglishWords(answer)}단어 · {formatTime(elapsed)} · 다시 듣기 {replays}회 · 힌트 {hints}회</p><p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-fg-muted">{answer}</p></div> : <p className="mt-4 text-sm text-fg-subtle">아직 답변하지 않았습니다.</p>}
    </div>}
  </Card>;
}
