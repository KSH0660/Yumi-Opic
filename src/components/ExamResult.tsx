...
export default function ExamResult({ exam, title, answers, times, onRetry, onRegenerate }: {
  ...
  return <main className="mx-auto w-full max-w-3xl px-5 pb-24 pt-8 sm:px-8">
    ...
    <Card className="animate-fade-up mt-5 p-6 sm:p-8">
      ...
      <p className="mt-2 text-sm leading-relaxed text-ink-300">문항별 질문과 작성한 답변을 확인하고 다시 말해 보세요.</p>
      ...
      <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={onRetry} className="rounded-xl border border-ink-700 px-4 py-2.5 text-sm text-ink-300">같은 문제 다시 풀기</button>{onRegenerate && <button type="button" onClick={onRegenerate} className="rounded-xl bg-accent-600 px-4 py-2.5 text-sm font-medium text-white">새 문제 뽑기</button>}</div>
    </Card>
    <h2 className="mt-10 text-sm font-semibold tracking-widest text-ink-400">문항별 답변·문제 유형</h2>
...
function ItemResult({ item, answer, elapsed }: { item: ExamItem; answer: string; elapsed: number }) {
  ...
  return <Card className="overflow-hidden">
    ...
    {open && <div className="border-t border-ink-800 px-5 py-5">
      ...
      <p className="mt-3 text-sm leading-relaxed text-ink-200">{item.question.en}</p>
      <p className="mt-2 text-xs leading-relaxed text-ink-500">{item.question.ko}</p>
