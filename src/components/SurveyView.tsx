...
import { defaultSettings, loadSettings, markSurveySeen, saveSurveyChoices } from "@/lib/storage";
...
export default function SurveyView() {
  ...
  useEffect(() => {
    // 아무것도 고치지 않고 나가도 홈이 다시 이 화면으로 되돌리지 않게 방문 사실을 남긴다.
    markSurveySeen();
    ...
    <section className="overflow-hidden rounded-2xl border border-line bg-white text-[#222] shadow-card">
      ...
      <div className="px-5 py-6 sm:px-8 sm:py-8">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#dedede] pb-5">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Background Survey</h1>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[#dedede] pt-6">
          ...
          {/* 실제 시험의 Step 1 다음처럼 한 걸음 나아가되, 연습 방식은 홈에서 고른다. */}
          <Link
            href={canContinue ? "/" : "#"}
