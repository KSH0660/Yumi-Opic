...
export function SourceBadge({ source = "adapted", sourceRef }: { source?: QuestionSource; sourceRef?: SourceReference }) {
  ...
  return <span title={descriptions[source]} className={`inline-flex cursor-help items-center rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${source === "verified" ? "bg-emerald-500/10 text-emerald-300 ring-emerald-500/25" : "bg-ink-800 text-ink-400 ring-ink-700"}`}>
