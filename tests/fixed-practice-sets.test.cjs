...
test('any survey topic can supply set order, arbitrary question IDs and actual slots through data alone', () => {
  ...
  const topic = { ...home, id: 'another-survey-topic', fixedPracticeSets: [
    { label: '첫 세트', items: [{ slot: 8, questionId: 'home-d2' }, { slot: 4, questionId: 'home-e1' }] },
    { label: '다음 세트', items: [{ slot: 14, questionId: 'home-c1' }] },
  ] };
  ...
  assert.deepEqual(first.items.map(i => [i.slot, i.question.id, i.comboLabel]), [
    [8, 'home-d2', '첫 세트'], [4, 'home-e1', '첫 세트'], [14, 'home-c1', '다음 세트'],
  ]);
  assert.equal(first.items[0].question, home.questions.find(q => q.id === 'home-d2'));
...
test('staycation retains its exact notice, mode and fixed slots', () => {
  ...
  assert.deepEqual(exam.items.map(i => i.slot), [2,3,4,5,6,7,11,12,13,14,15]);
  assert.deepEqual(exam.items.map(i => itemNumber(exam.mode, i)), ['2','3','4','5','6','7','11','12','13','14','15']);
  assert.deepEqual(exam.notices, ['Q2–Q4 / Q5–Q7 / Q11–Q13 / Q14–Q15 순서로 연습합니다. 원하는 문항만 답변할 수 있습니다.']);
});
...
test('an unconfigured topic retains 14-question practice with the original type-based draws', () => {
  assert.equal(home.fixedPracticeSets, undefined);
  const first = buildPracticeExam(home, () => 0);
  ...
  assert.deepEqual(first.items.map(i => i.question.id), [
    'home-d1','home-r1','home-e1','home-d1','home-e1','home-m1','home-d1',
    'home-e1','home-m1','home-rp11','home-rp12','home-rp13','home-c1','home-i1',
  ]);
  assert.equal(buildPracticeExam(home, () => 0.99).items[0].question.id, 'home-d2');
});

test('invalid fixed sets fail instead of silently dropping questions or overwriting answer slots', () => {
  const entry = { slot: 2, questionId: 'home-d1' };
...
test('single practice keeps drawing from the shared bank; set practice requires complete declared sets', () => {
  const configured = { ...home, fixedPracticeSets: [{ label: '고정', items: [{ slot: 14, questionId: 'home-c1' }] }] };
  assert.throws(() => buildTopicSet([configured], () => 0));
  for (const build of [buildSingleQuestion]) {
    assert.deepEqual(build([configured], () => 0).items, build([home], () => 0).items);
    assert.deepEqual(build([configured], () => 0.99).items, build([home], () => 0.99).items);
  }
});
